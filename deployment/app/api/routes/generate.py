...
def _validate_generate_prompt(req: GenerateRequest) -> None:
    ...
    if (
        sum(
            [
                1 if has_wav else 0,
                1 if has_latents else 0,
                1 if has_prompt_id else 0,
                1 if has_hifi_id else 0,
            ]
        )
        > 1
    ):
    ...
    if (
        sum(
            [
                1 if has_ref_wav else 0,
                1 if has_ref_latents else 0,
                1 if has_ref_id else 0,
            ]
        )
        > 1
    ):
    ...
    if has_prompt_id:
        if req.prompt_text not in (None, ""):
            raise HTTPException(
                status_code=400,
                detail="prompt_text is not allowed when prompt_id is set",
            )
        return
...
async def _resolve_generation_context(
    ...
    server: Any,
    feat_dim: int,
    hifi_pool: dict[str, dict[str, str]] | None = None,
) -> tuple[bytes | None, bytes | None, str, str | None, str | None]:
...
async def _make_wav_chunks(
    ...
    generate_kwargs = _build_generate_kwargs(
        req, prompt_latents, prompt_text, prompt_id, ref_audio_latents, ref_audio_id
    )
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Reference audio is not supported by the loaded model: {e}",
        ) from e
...
async def generate(
    ...
    async def instrumented_wav_chunks() -> AsyncIterator[NDArray[np.float32]]:
        ...
        async for chunk in _make_wav_chunks(
            req,
            server,
            prompt_latents,
            prompt_text,
