...
RUNNER_TRACE_ENABLED = os.environ.get(
...
class VoxCPM2Runner(BaseModelRunner):
    ...
    def make_dummy_outputs(self, batch_size: int) -> dict[str, torch.Tensor]:
        return {
            "latents": torch.zeros(
                batch_size, self.patch_size, self.feat_dim, dtype=self.dtype
            ),
    ...
    def run(self, seqs: list[RunnerTask[VoxCPM2Payload]], is_prefill: bool):
        ...
        pad_lengths = [
            seq.custom_payload.padding_decode.shape[0]
            if seq.custom_payload.padding_decode is not None
            else 0
            for seq in seqs
        ]
        ...
        for i, seq in enumerate(seqs):
            ...
            if pad_len > 0:
                vae_decoder_inputs[i, :pad_len] = torch.from_numpy(
                    seq.custom_payload.padding_decode
                ).cuda(non_blocking=True)
