...
def add_reference(base_url: str, ref_b64: str, wav_format: str) -> tuple[str, float]:
    t0 = time.time()
    r = requests.post(base_url + '/add_reference', json={'wav_base64': ref_b64, 'wav_format': wav_format}, timeout=120)
    r.raise_for_status()
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--prompt-text', default=None, help='Prompt text for add_prompt; default=same as --text')
    p.add_argument('--ref-wav', default=None, help='Reference wav path; default=latest /tmp/gradio/*/audio.wav')
    p.add_argument('--wav-format', default='wav')
    p.add_argument('--runs', type=int, default=2)
    p.add_argument('--do-warmup', action='store_true', help='Run a tiny generate_blocking_wav first to keep service hot')
    return p.parse_args()
...
def main() -> None:
    ...
    out['routes']['ref_audio_id'] = {'add_sec': add_ref_sec, 'delete_status': ref_delete, 'runs': ref_runs}
    ...
    out['routes']['prompt_id'] = {'add_sec': add_prompt_sec, 'delete_status': prompt_delete, 'runs': prompt_runs}
