...
def create_demo() -> gr.Blocks:
    ...
    with gr.Blocks(css=CUSTOM_CSS) as ui:
        ...
        gr.Markdown(
            "上传参考音频后，可先点 **ASR 自动填充**，再手动修正 prompt text，最后生成 HiFi 克隆结果。"
        )
        with gr.Row():
            with gr.Column():
                ...
                with gr.Accordion("高级参数", open=False):
                    ...
                    max_generate_length = gr.Slider(64, 4096, value=DEFAULT_MAX_GENERATE_LENGTH, step=1, label="max_generate_length")
                run_btn = gr.Button("开始生成", variant="primary", size="lg")
            with gr.Column():
                audio_out = gr.Audio(label="生成结果")
                meta_out = gr.Textbox(label="本次请求参数", lines=8)

        asr_btn.click(fn=demo.recognize, inputs=[ref_wav], outputs=[prompt_text], show_progress=True)
        run_btn.click(
            fn=demo.generate_hifi,
            inputs=[ref_wav, prompt_text, target_text, cfg_value, temperature, max_generate_length],
