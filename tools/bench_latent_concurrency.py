...
def gpu_snapshot() -> dict[str, Any]:
    ...
    try:
        out = run_cmd(["nvidia-smi", "--query-gpu=memory.used,utilization.gpu", "--format=csv,noheader,nounits"]).splitlines()
        if out:
    ...
    try:
        out = run_cmd(["nvidia-smi", "--query-compute-apps=pid,process_name,used_gpu_memory", "--format=csv,noheader,nounits"]).splitlines()
        for line in out:
            if not line.strip():
                continue
    ...
    def _run(self):
        while not self._stop.is_set():
            try:
                ...
                if out:
                    mem, util = [x.strip() for x in out[0].split(",")[:2]]
                    self.samples.append({"ts": time.time(), "memory_used_mib": float(mem), "util_gpu_pct": float(util)})
            except Exception:
                pass
...
def encode_latents(base_url: str, ref_b64: str, wav_format: str):
    t0 = time.time()
    r = requests.post(base_url + "/encode_latents", json={"wav_base64": ref_b64, "wav_format": wav_format}, timeout=(CONNECT_TIMEOUT, READ_TIMEOUT))
    r.raise_for_status()
...
def one_request(idx: int, base_url: str, payload: dict, barrier: threading.Barrier):
    ...
    total_bytes = 0
    try:
        with requests.post(base_url + "/generate", json=payload, stream=True, timeout=(CONNECT_TIMEOUT, READ_TIMEOUT)) as r:
            r.raise_for_status()
        ...
        t1 = time.time()
        return {"ok": True, "idx": idx, "ttfb_sec": None if first is None else round(first - t0, 3), "total_sec": round(t1 - t0, 3), "bytes": total_bytes}
    except Exception as e:
        t1 = time.time()
        return {"ok": False, "idx": idx, "ttfb_sec": None if first is None else round(first - t0, 3), "total_sec": round(t1 - t0, 3), "bytes": total_bytes, "error": repr(e)}
...
def summarize(name, results, batch_wall, baseline_duration, samples, before_gpu):
    ...
    return {
        ...
        'approx_avg_rtf': None if not totals or not baseline_duration else round(statistics.mean(totals) / baseline_duration, 3),
        'approx_p95_rtf': None if not totals or not baseline_duration else round(percentile(totals, 0.95) / baseline_duration, 3),
...
def parse_args():
    ...
    p.add_argument('--ref-wav', default=None, help='Path to reference wav. Default: latest /tmp/gradio/*/audio.wav')
...
def main():
    ...
    payload = {'target_text': args.text, 'ref_audio_latents_base64': enc['prompt_latents_base64']}

    print(json.dumps({'stage': 'setup', 'text': args.text, 'text_len': len(args.text), 'reference_wav': ref, 'reference_duration_sec': ffprobe_duration(ref), 'gpu_before': gpu_snapshot(), 'encode_latents_sec': round(enc_sec, 3)}, ensure_ascii=False), flush=True)
    ...
    for c in args.concurrency:
        ...
        mon.stop()
        summary = summarize(f'latent_concurrency_{c}', results, batch_wall, baseline_duration, mon.samples, before)
        rounds.append(summary)
        print(json.dumps({'stage': 'round', **summary}, ensure_ascii=False), flush=True)
        time.sleep(3)

    print(json.dumps({'stage': 'done', 'gpu_after': gpu_snapshot(), 'rounds': rounds}, ensure_ascii=False, indent=2), flush=True)
